...
|------|----------|---------|-----------------|--------|
| 2026-08-30 | `AsyncExecutor` submission overhead vs raw `CompletableFuture.supplyAsync` — the NFR-02 budget | v0.1.0 + item 5.2 | **Met by ~5000x: the overhead is at or below the harness's 1 ns resolution against a 5 us ceiling.** The first harness written measured ~12 us on every arm including the raw one — that was a thread handoff, not submission — so the budget is measured inline and the pooled pair is reported separately | [report](2026-08-30-async-submission.md) |
