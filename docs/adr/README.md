...
Both indexes below are complete: **33 decisions, no third home.** `consistency_lint.py`'s `adr-index`
...
instead of silently absorbed. Item 4.1 took it from 27 to **29**, item 4.2 to **31** and item 4.3 to
**33** — two records each time, and the count is maintained by hand on every pass, which is the
standing reason it drifts.)
