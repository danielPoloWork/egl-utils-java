...
The mechanical step-by-step for cutting a release of `egl-utils-java`. The governance
...
1. **Bump the version constant** (<version>X.Y.Z</version>) in `pom.xml`; update any
   version-check test.
