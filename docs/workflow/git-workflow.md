...
Full conventions for branches, commits, and pull requests on `egl-utils-java`. The short
...
- `scope` is a short subsystem noun. Scopes for this repo: `core` `jdbc` `concurrent` `security` `json` `spring` `redisson` `testkit` `bom` `build` `tests` `docs` `ci` .
