...
| SQL altered via string concatenation | B1 · `SimpleJdbcExecutor` (FR-05) | **Closed in item 4.3** ([ADR-0028](../adr/0028-the-fr-05-operation-set-and-what-it-refuses.md)). Defended **by construction**, and the claim is now structural rather than editorial: every operation is `(String sql, ..., Object... params)` bound through a `PreparedStatement`, there is no overload without a parameter slot, and **no `java.sql.Statement` is created anywhere in the module** — a zero-parameter call is still a prepared statement. Both halves are asserted by running rather than by reading: `treatsAnInjectionPayloadAsData` binds `'; drop table orders; --` and finds it stored as a string with the table intact, and `neverCreatesAPlainStatement` runs every operation through a `Connection` proxy that fails the test the moment `createStatement` is called, so a concatenating overload added later cannot pass. `publishesNoWayToRunUnparameterizedSql` walks the reflected surface and fails on any public method that takes SQL without a trailing `Object[]`. **Residual, stated:** a caller that concatenates the `sql` string *before* passing it in is beyond what any Java API can stop; what this one removes is the path where concatenation is the convenient option. A column or table name cannot be a bind parameter in any database, which is what FR-07's allowlist is for (item 4.5, the row below) | ✅ |
| Injection via `ORDER BY` — a column name cannot be a bind parameter | B1 · `PageRequest` (FR-07) | **Closed in item 4.5** ([ADR-0033](../adr/0033-publish-no-accessor-for-the-unvalidated-sort.md)), and the row's own text needed correcting on the way: violations do **not** throw "at construction". RFC-0003 §FR-07 moved the whitelist to validation time — a `PageRequest` is built at the HTTP edge and the set of sortable columns is the repository's knowledge — and the exception is a **`ValidationException`**, not `IllegalArgumentException`, because FR-19 maps validation to 400 and has no row for the latter, which would report a client's typo as a 500. **The control is structural rather than editorial, which is what earns the ✅:** `PageRequest` publishes **no accessor that returns the sort**, so `validatedAgainst(Set<String>)` — which takes the allowlist as its argument — is the only route to the client's property strings at all. A repository cannot skip the check, because there is nothing to read without it. `publishesNoAccessorThatReturnsTheSort` walks the reflected surface and fails on any method returning a `PageSort` that does not take a `Set`, so a convenience accessor added later cannot pass. `PageRequest.toString()` renders the sort as a **count**, never the names, since a formatting method would otherwise restore by another door what the missing accessor removes. **Both demonstrations run against a real database and neither is inert:** `theSamePayloadDropsTheTableWhenNobodyValidatedIt` concatenates `sku; drop table orders --` into the clause and the table is gone afterwards — through `SimpleJdbcExecutor.query` and a `PreparedStatement`, because a statement string holding a semicolon is still one string — while `theSamePropertyLeaksAHiddenColumnWhenNobodyValidatedIt` shows the quieter and more likely attack: ordering by `note`, a column the allowlist omits and no query selects, reorders the rows by its values, so one comparison per request extracts it. Comparison is **exact and case-sensitive** and that turns out to be load-bearing twice over — RFC-0003 refused normalisation because identifier folding is vendor-specific, and exact matching additionally means the surviving string is `equals` to the one the repository listed, so interpolating the client's is indistinguishable from interpolating our own. **Residual, stated:** a component that both parses the request and builds the SQL still holds the `PageSort` it constructed; what is removed is the case where the two are different layers, which is every application with a repository | ✅ |
...
| A pooled worker thread carrying one task's context into the next | B1 · `AsyncExecutor` (FR-09) | **Row added by [RFC-0004](../rfc/0004-concurrency-contracts.md) §FR-09, which found the section had eight rows and none for this.** The interesting failure of context propagation is not the missing context but the **surviving** one: a worker thread is pooled and reused, so a task that installs a context and does not put back what was there leaves it visible to the next task on that thread — a different request, often a different user or tenant, whose log lines then carry someone else's identifiers. The control is the SPI's shape rather than a caller's discipline: `ContextSnapshot.install()` returns a `Scope` that **restores the previous value**, and clearing is refused because it is wrong in both directions — it wipes context a caller had already established for nested submissions, and it does nothing about the ordering that actually leaks. The invariant is stated normatively: *every installed snapshot is closed on the same thread, in the reverse order of installation, including when the task throws, so a worker thread's context after a task is exactly what it was before*. Item 5.2's contract test is a **single-threaded** pool and two tasks with distinct contexts, asserting the second sees nothing of the first — a sequential property, which is why this is not one of the jcstress harnesses spec §6 requires. Security-relevant under the enterprise posture, so item 5.2 carries an ADR (AGENTS.md §7) | ▢ |

### Denial of service — can the surface be exhausted?

| Threat considered | Boundary / component | Mitigation / control | Status |
|---|---|---|---|
| Unbounded page size exhausting memory on a large table | B1 · `PageRequest` (FR-07) | **Closed in item 4.5.** `1 ≤ size ≤ maxSize` with the specification's default of 200, and "configurable" is a **parameter** — `of(page, size, sort, maxSize)` — never a system property or a static field, because a host-global default cannot be tested in parallel and makes one call return different results in two modules of one application. These bounds *do* throw `IllegalArgumentException` at construction, as spec §2 FR-07 states normatively, and the asymmetry with the row above is real and is **filed on item 7.1** rather than resolved by an implementation quietly disagreeing with an approved RFC: FR-19 has no `IllegalArgumentException` row, so an out-of-range `?page=-1` reaching the constructor unmediated lands on the 500 fallback. The `@apiNote` on `PageRequest.of` states the caller obligation — bind and range-check at the edge, which every framework in the matrix does declaratively. **A second unbounded input the requirement does not name is bounded here:** a sort may name at most **8** properties, checked at *validation* so a client repeating `?sort=` gets a 400 rather than a 500, because the allowlist bounds *which* columns and never *how many* — a repository with fifty sortable columns would otherwise accept a fifty-column `ORDER BY`. A property named twice is refused for the same reason (`ORDER BY sku ASC, sku DESC` is legal SQL whose second clause is dead). `PageResponse` imposes no cap of its own and does not need one: it holds at most `size` rows, asserted at construction | ✅ |
