...
| Threat considered | Boundary / component | Mitigation / control | Status |
|---|---|---|---|
| A state change cannot be attributed after the fact | B1 · `AuditLog` (FR-16) | who / when / before-after values recorded | ▢ |
| The audit trail itself becomes the leak — secrets and PII faithfully recorded into a store retained longer and replicated wider than app logs | B1 · `AuditLog` | **No redaction policy specified.** Needs field-level allowlisting, a `@Sensitive` opt-out and a never-capture list. Scheduled to item 3.3, and RFC-0002 (item 3.0) must settle it *before* any code | ⚠ |
