...
Single source of truth for AI coding agents working on `egl-utils-java`. This file is
...
You are a **senior project architect with 20+ years of professional Java 21 (LTS)
...
- Default to **standards-compliant Java 21 (LTS) toolchain; --release 17 language/bytecode baseline**. Avoid non-standard extensions
...
- Prefer **measurable correctness** (unit tests, ErrorProne/NullAway (compile-time soundness), jcstress (concurrency), JFR leak profiling, OWASP Dependency-Check, benchmarks) over
...
- Scopes for this repo: `core` `jdbc` `concurrent` `security` `json` `spring` `redisson` `testkit` `bom` `build` `tests` `docs` `ci` .
...
- **Concurrency:** every shared-state type documents whether it is thread-safe;
  data races are forbidden and verified by the CI race/sanitizer job (ErrorProne/NullAway (compile-time soundness), jcstress (concurrency), JFR leak profiling, OWASP Dependency-Check).
...
| Gate | Requirement |
|---|---|
| Build matrix | Linux / Windows / macOS on Temurin JDK 17 & 21 — every CI cell green |
| Warnings | zero, treated as errors on the diff |
| Lint | `ErrorProne + NullAway + Checkstyle; maven-enforcer for the ADR-001 dependency rules` clean on the diff; no broad disables |
| Format | `Spotless (google-java-format)` clean |
| Unit tests | cover the new/changed behavior; pass on every CI cell |
| Sanitizers / checkers | ErrorProne/NullAway (compile-time soundness), jcstress (concurrency), JFR leak profiling, OWASP Dependency-Check green where applicable |
| Coverage | new code ≥ 85% line (finalized in an ADR) |
| API docs | `Javadoc` builds without warnings |
...
- **Agent-vs-human boundary** mirrors §6.1: the agent bumps the version constant
  (`<version>X.Y.Z</version>` in `pom.xml`), rolls the changelog, drafts release
