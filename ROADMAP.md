...
- [ ] 1.8 JMH (`bench/`) and jcstress (`jcstress/`) harness skeletons so the NFR gates have somewhere to land — **and declare the `jmh` and `jcstress` Maven profiles the CI jobs already invoke.** Discovered while closing 1.2: `ci.extra_jobs` runs `mvn -B -Pjmh verify` and `mvn -B -Pjcstress verify`, but neither profile exists, and Maven only *warns* on an unmatched profile (`The requested profile "jmh" could not be activated because it does not exist`). So both jobs currently report **pass while verifying nothing** — the same vacuous-green class as the `version-lockstep` defect 1.1 fixed. Until this lands, treat those two green checks as meaningless — size: **S** · route: **standard / medium**
