...
- [x] 1.4 Formatter and linter configs at the reactor root — Spotless 2.44.5 (google-java-format 1.27.0), ErrorProne 2.50.0 + NullAway 0.12.7 as javac plugins **confined to a JDK 21+ profile** (forced, not preferred: ErrorProne 2.50.0 is class-file 65 and cannot execute on the Temurin 17 job at all, and on 21 it refuses to start without the should-stop=ifError=FLOW flag — CI proved both). Nothing is lost from the artifact: every module still compiles with release 17 on every platform, and analysis findings surface on the three JDK 21 jobs, Checkstyle 10.26.1 via plugin 3.6.0 — inherited by every module from the parent POM. Spotless and Checkstyle are declared in `<build><plugins>`, not only in `pluginManagement`: Maven resolves a goal prefix from `<build><plugins>` plus the default pluginGroups, and `com.diffplug.spotless` is in neither, so a management-only entry reproduces the very `No plugin found for prefix 'spotless'` failure this item exists to fix. `.mvn/jvm.config` carries the ten `jdk.compiler` export flags ErrorProne needs from JDK 16 on. **Division of labour is deliberate:** Spotless owns formatting, `config/checkstyle/checkstyle.xml` owns only what a formatter cannot see — naming, import legality, structure, public-API Javadoc — and encodes the spec §1.1 jakarta-only policy as an `IllegalImport` rule that still permits JDK `javax.crypto`/`javax.sql` (FR-12, FR-05 need them). `google_checks.xml` was rejected: it governs formatting Spotless already owns, so the two would fight over the same lines — size: **M** · route: **standard / high**
