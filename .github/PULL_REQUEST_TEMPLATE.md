...
- [ ] Builds cleanly on the full CI matrix (Linux / Windows / macOS on Temurin JDK 17 & 21)
- [ ] Unit tests pass; new/changed behavior covered (≥ 85% line)
- [ ] `Spotless (google-java-format)` clean; `ErrorProne + NullAway + Checkstyle; maven-enforcer for the ADR-001 dependency rules` clean on the diff
- [ ] ErrorProne/NullAway (compile-time soundness), jcstress (concurrency), JFR leak profiling, OWASP Dependency-Check green (where applicable)
