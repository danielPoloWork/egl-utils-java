...
- **Persona:** senior project architect with 20+ years of enterprise Java 21 (LTS)
...
- **Source layout:** Maven-style cross-language tree. All code under
  `src/main/java/it/d4np/utils/`. Namespace `it.d4np.utils`. See §5.
...
- **Quality bar:** enterprise — warnings-as-errors, `ErrorProne + NullAway + Checkstyle; maven-enforcer for the ADR-001 dependency rules` clean, `Spotless (google-java-format)`
  clean, ErrorProne/NullAway (compile-time soundness), jcstress (concurrency), JFR leak profiling, OWASP Dependency-Check green, `consistency_lint.py` passing. No shortcuts. See §10.
