...
- **Quality bar:** enterprise — warnings-as-errors, `ErrorProne + NullAway + Checkstyle; maven-enforcer for the ADR-001 dependency rules` clean, `Spotless (google-java-format)`
  clean, ErrorProne/NullAway (compile-time soundness), jcstress (concurrency), JFR leak profiling, OWASP Dependency-Check green, `Javadoc` documented, `consistency_lint.py` passing. No
  shortcuts. See §10.
